...
/// <summary>Liest und schreibt Ren'Py-Archive (.rpa). Format-Referenz: Ren'Py
/// <c>loader.py</c>, <c>unrpa</c> und <c>rpatool</c>.
///
/// <para>Archiv-Aufbau:</para>
/// <list type="number">
/// <item>Header-Zeile (ASCII, endet mit <c>\n</c>):
///   <list type="bullet">
///   <item><c>RPA-3.0 &lt;indexOffset:16hex&gt; &lt;key:8hex&gt;\n</c></item>
///   <item><c>RPA-3.2 &lt;indexOffset:16hex&gt; 0 &lt;key:8hex&gt;\n</c></item>
///   <item><c>RPA-2.0 &lt;indexOffset:16hex&gt;\n</c> (kein Key)</item>
///   </list></item>
/// <item>Danach die rohen Dateidaten.</item>
/// <item>Ab <c>indexOffset</c>: zlib-komprimiertes Python-Pickle des Index.
///   Index = <c>dict{ Dateiname : [ [offset, length, (prefix)] , ... ] }</c>.
///   Bei 3.0/3.2 sind offset und length mit dem Key XOR-verschleiert.</item>
/// </list>
///
/// <para>Portiert aus RenPack (<c>Services/RenpyArchiveService.cs</c>).</para></summary>
public sealed class RenpyArchiveService
{
    ...
    public RpaArchiveInfo ReadIndex(string archivePath)
    {
        ...
        switch (parts[0])
        {
            ...
            default:
                throw new NotSupportedException(
                    $"Nicht unterstütztes Format '{parts[0]}'. Unterstützt: RPA-2.0/3.0/3.2.");
        }
        ...
        foreach (DictionaryEntry de in dict)
        {
            ...
            foreach (var segObj in AsList(de.Value))
            {
                ...
                byte[] prefix = tuple.Count > 2 ? ToBytes(tuple[2]) : Array.Empty<byte>();
    ...
    /// <summary>Liest einen Entry als Byte-Array (für Preview). Rückgabe null
    /// wenn Größe <paramref name="maxBytes"/> überschreitet — spart RAM bei
    /// versehentlichem Klick auf große Assets.</summary>
    ...
    private void ExtractEntryCore(Stream archive, RpaEntry entry, string destinationFile)
    {
        ...
        foreach (var seg in entry.Segments)
        {
            ...
            while (remaining > 0)
            {
                ...
                if (read <= 0)
                    throw new EndOfStreamException(
                        $"Archiv unerwartet zu Ende beim Entpacken von '{entry.Path}'.");
                output.Write(buffer, 0, read);
                remaining -= read;
            }
        }
    ...
    private static string ReadHeaderLine(Stream s)
    {
        ...
        while ((b = s.ReadByte()) != -1)
        {
            ...
            bytes.Add((byte)b);
            if (bytes.Count > 4096)
                throw new NotSupportedException("Kein gültiger RPA-Header gefunden.");
        }
    ...
    /// <summary>Kombiniert Zielordner + Archiv-Pfad und verhindert Ausbrüche
    /// (<c>../</c>, absolute Pfade).</summary>
    private static string SafeCombine(string root, string relativePath)
    {
        ...
        if (!combined.StartsWith(rootWithSep, StringComparison.Ordinal) &&
            ...
            throw new IOException($"Archiv-Eintrag '{relativePath}' zeigt aus dem Zielordner heraus.");
        return combined;
    }
