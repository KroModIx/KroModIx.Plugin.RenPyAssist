...
public sealed class KrosteWalkthroughGenerator
{
        ...
        IReadOnlyList<(string OriginalText, string Hint)> hints)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    krostemod_walkthrough_call_count = 0");
        sb.AppendLine();
        sb.AppendLine("    def _krostemod_apply_walkthrough_hints(*_args, **_kw):");
        sb.AppendLine("        global krostemod_walkthrough_call_count");
        sb.AppendLine("        krostemod_walkthrough_call_count += 1");
        sb.AppendLine("        call_num = krostemod_walkthrough_call_count");
        sb.AppendLine("        # print() geht in Ren'Py's log.txt (stdout capture).");
        sb.AppendLine("        # renpy.log ist Attribut, keine Funktion — deshalb print.");
        sb.AppendLine("        print('[krostemod] walkthrough patcher call #{}'.format(call_num))");
        sb.AppendLine("        try:");
        sb.AppendLine("            translator = renpy.game.script.translator");
        sb.AppendLine("            strings = translator.strings");
        sb.AppendLine("        except Exception as e:");
        sb.AppendLine("            print('[krostemod] no translator access: {}'.format(e))");
        sb.AppendLine("            if call_num == 1:");
        sb.AppendLine("                renpy.notify('krostemod: kein Translator-Zugriff (siehe log.txt)')");
        sb.AppendLine("            return");
        sb.AppendLine("        patched = 0");
        sb.AppendLine("        langs_found = []");
        sb.AppendLine("        all_langs = list(strings.keys()) if hasattr(strings, 'keys') else []");
        sb.AppendLine("        print('[krostemod] configured langs: {}, available in translator: {}'.format(");
        sb.AppendLine("            krostemod_walkthrough_langs, all_langs))");
        sb.AppendLine("        # Wir versuchen unsere konfigurierten Sprachen UND alle die im");
        sb.AppendLine("        # Translator vorhanden sind — falls die Sprach-Codes anders sind.");
        sb.AppendLine("        targets = set(krostemod_walkthrough_langs) | set(all_langs)");
        sb.AppendLine("        for lang in targets:");
        sb.AppendLine("            stl = strings.get(lang) if hasattr(strings, 'get') else None");
        ...
        sb.AppendLine("        print('[krostemod] patched {} entries across {} langs: {}'.format(");
        sb.AppendLine("            patched, len(langs_found), langs_found))");
        sb.AppendLine("        # Beim ersten Aufruf einen Notify anzeigen (User-visible).");
        sb.AppendLine("        if call_num == 1:");
        sb.AppendLine("            try: renpy.notify('KrosteMod Walkthrough: {} Hints in {} Sprachen aktiv'.format(patched, len(langs_found)))");
        ...
        sb.AppendLine();
        sb.AppendLine("    # ZUSAETZLICH: hook in start_callbacks/after_load_callbacks, damit");
        sb.AppendLine("    # bei jedem Save-Load und Spiel-Neustart der Patch neu greift");
        sb.AppendLine("    # (Ren'Py laed translations pro Session frisch).");
        sb.AppendLine("    if hasattr(config, 'start_callbacks'):");
        sb.AppendLine("        if _krostemod_apply_walkthrough_hints not in config.start_callbacks:");
        sb.AppendLine("            config.start_callbacks.append(_krostemod_apply_walkthrough_hints)");
        sb.AppendLine("    if hasattr(config, 'after_load_callbacks'):");
        sb.AppendLine("        if _krostemod_apply_walkthrough_hints not in config.after_load_callbacks:");
        sb.AppendLine("            config.after_load_callbacks.append(_krostemod_apply_walkthrough_hints)");
